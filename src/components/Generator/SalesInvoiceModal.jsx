...
import { 
  ...
  BsClockHistory, BsPalette, BsCalendarCheck, BsCalendarEvent, BsArrowRepeat, BsFileImage, BsChevronDown, BsChevronUp
} from "react-icons/bs";
...
export default function SalesInvoiceModal({ isOpen = true, onClose, isPage = false, initialStage = "QUOTATION" }) {
  const { activeWorkbench } = useWorkbench();
  const { user } = useAuth();
  const company = getWorkbenchCompanyDetails(activeWorkbench, user);

  const [stage, setStage] = useState(initialStage || "QUOTATION");
  const [docNumber, setDocNumber] = useState(`QT-${Math.floor(1000 + Math.random() * 9000)}`);
  const [docDate, setDocDate] = useState(new Date().toISOString().split("T")[0]);
  const [dueDate, setDueDate] = useState("");

  // Company Logo & Letterhead Customization Profile
  const [showLetterheadConfig, setShowLetterheadConfig] = useState(false);
  const [letterhead, setLetterhead] = useState(() => {
    const saved = localStorage.getItem("dabby_letterhead_profile");
    if (saved) {
      try { return JSON.parse(saved); } catch (e) {}
    }
    return {
      logoUrl: company.logo || "/dabby-logo.svg",
      companyName: company.legalName || company.name || "DATALIS TECHNOLOGIES PVT LTD",
      tagline: company.tagline || "AI Business Intelligence & Enterprise Billing",
      address: company.address || "Plot 42, MIDC Industrial Area, Mumbai, MH - 400093",
      gstin: company.gstin || "27AABCU9603R1ZM",
      pan: company.pan || "AABCU9603R",
      phone: company.phone || "9870048082",
      email: company.email || "billing@datalis.in",
      placeOfSupply: "Maharashtra",
      themeColor: "#05B8A5",
      authorisedSignatory: company.legalName || company.name || "Authorized Signatory",
      signatureUrl: "",
      customFooterNotes: "Thank you for doing business with us. Computer-generated tax invoice."
    };
  });

  const handleLetterheadChange = (field, value) => {
    setLetterhead((prev) => {
      const updated = { ...prev, [field]: value };
      localStorage.setItem("dabby_letterhead_profile", JSON.stringify(updated));
      return updated;
    });
  };

  // Auto-Scheduled Recurring Billing Configuration
  const [showScheduledBilling, setShowScheduledBilling] = useState(false);
  const [scheduledBilling, setScheduledBilling] = useState({
    isEnabled: false,
    frequency: "MONTHLY", // MONTHLY | QUARTERLY | WEEKLY | BI_WEEKLY | ANNUAL
    nextBillingDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
    autoSendEmail: true,
    maxBillingCycles: 12,
    status: "ACTIVE",
    notes: "Auto-recurring invoice schedule generated by Dabby AI Business Engine"
  });
  ...
  // PDF Exporter with auto-fitted Workbench company & custom Letterhead settings
  const handleExportPDF = () => {
    try {
      ...
      const doc = generateStandardDocumentPDF({
        ...
        docDate: docDate,
        dueDate: dueDate,
        senderName: letterhead.companyName || company.legalName || company.name,
        senderAddress: letterhead.address || company.address,
        senderGstin: letterhead.gstin || company.gstin,
        senderPan: letterhead.pan || company.pan,
        senderCin: company.cin,
        senderEmail: letterhead.email || company.email,
        logo: letterhead.logoUrl || company.logo,
        ...
        columnLabels,
        showSeparateUnitCol,
        notes: letterhead.customFooterNotes || "T-Patti for top,bottom & center support\n2\"X 2\" Pipe Ms Fabrication For Fins Support",
        terms: paymentSnippet.paymentTerms || "50% Advance\n30% ongoing work\n20% after completion",
        bankDetails: {
          name: paymentSnippet.accountName || letterhead.companyName,
        ...
        authorisedSignatory: letterhead.authorisedSignatory || company.legalName || company.name
      });
  ...
  const handleSaveDocument = async () => {
    ...
    try {
      ...
      const pdfDoc = generateStandardDocumentPDF({
        ...
        columnLabels,
        showSeparateUnitCol,
        notes: letterhead.customFooterNotes || "",
      ...
      await saveDocumentToDocVaultAndEngine({
        ...
        scheduledBilling: scheduledBilling.isEnabled ? scheduledBilling : null
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md font-dm-sans overflow-y-auto">
      <div className="bg-[#121212] border border-white/10 rounded-2xl w-full max-w-5xl my-8 overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 space-y-6 overflow-y-auto flex-1">
          ...
          {/* Logo & Letterhead Customization Accordion */}
          <div className="rounded-xl bg-white/5 border border-white/5 overflow-hidden transition-all">
            <button
              type="button"
              onClick={() => setShowLetterheadConfig(!showLetterheadConfig)}
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="text-sm font-bold text-white flex items-center gap-2">
                    Company Logo & Letterhead Settings
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-teal-500/20 text-teal-300 font-mono">
                      Customizable PDF Header
                    </span>
                  </div>
                  <div className="text-xs text-gray-400">
                    Customize logo, business name, GSTIN, letterhead headers & authorised signatory stamp
                  </div>
                </div>
              </div>
              <div className="flex items-center space-x-2 text-gray-400">
                <span className="text-xs text-teal-400 font-medium">
                  {showLetterheadConfig ? "Hide Settings" : "Configure Letterhead"}
                </span>
                {showLetterheadConfig ? <BsChevronUp /> : <BsChevronDown />}
              </div>
            </button>

            {showLetterheadConfig && (
              <div className="p-5 border-t border-white/10 space-y-4 animate-in slide-in-from-top-2 duration-200">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  <div>
                    <label className="block font-semibold text-gray-300 mb-1">Company / Business Legal Name</label>
                    <input
                      type="text"
                      value={letterhead.companyName}
                      onChange={(e) => handleLetterheadChange("companyName", e.target.value)}
                      className="w-full bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white focus:border-teal-500"
                      placeholder="e.g. DATALIS TECHNOLOGIES PVT LTD"
                    />
                  </div>

                  <div>
                    <label className="block font-semibold text-gray-300 mb-1">Business Tagline / Subtitle</label>
                    <input
                      type="text"
                      value={letterhead.tagline}
                      onChange={(e) => handleLetterheadChange("tagline", e.target.value)}
                      className="w-full bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white focus:border-teal-500"
                      placeholder="e.g. AI Business Intelligence & Enterprise Billing"
                    />
                  </div>

                  <div className="md:col-span-2">
                    <label className="block font-semibold text-gray-300 mb-1">Company Logo URL / Asset Path</label>
                    <div className="flex items-center gap-2">
                      <input
                        type="text"
                        value={letterhead.logoUrl}
                        onChange={(e) => handleLetterheadChange("logoUrl", e.target.value)}
                        className="flex-1 bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white focus:border-teal-500"
                        placeholder="/dabby-logo.svg or https://..."
                      />
                      {letterhead.logoUrl && (
                        <div className="h-9 w-9 rounded-lg bg-black/40 border border-white/10 p-1 flex items-center justify-center shrink-0">
                          <img src={letterhead.logoUrl} alt="Logo Preview" className="max-h-full max-w-full object-contain" />
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="block font-semibold text-gray-300 mb-1">Company GSTIN</label>
                    <input
                      type="text"
                      value={letterhead.gstin}
                      onChange={(e) => handleLetterheadChange("gstin", e.target.value)}
                      className="w-full bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white uppercase focus:border-teal-500"
                    />
                  </div>

                  <div>
                    <label className="block font-semibold text-gray-300 mb-1">Company PAN Number</label>
                    <input
                      type="text"
                      value={letterhead.pan}
                      onChange={(e) => handleLetterheadChange("pan", e.target.value)}
                      className="w-full bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white uppercase focus:border-teal-500"
                    />
                  </div>

                  <div className="md:col-span-2">
                    <label className="block font-semibold text-gray-300 mb-1">Registered Billing Address</label>
                    <textarea
                      rows={2}
                      value={letterhead.address}
                      onChange={(e) => handleLetterheadChange("address", e.target.value)}
                      className="w-full bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white focus:border-teal-500"
                    />
                  </div>

                  <div>
                    <label className="block font-semibold text-gray-300 mb-1">Contact Email</label>
                    <input
                      type="email"
                      value={letterhead.email}
                      onChange={(e) => handleLetterheadChange("email", e.target.value)}
                      className="w-full bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white focus:border-teal-500"
                    />
                  </div>

                  <div>
                    <label className="block font-semibold text-gray-300 mb-1">Contact Phone</label>
                    <input
                      type="text"
                      value={letterhead.phone}
                      onChange={(e) => handleLetterheadChange("phone", e.target.value)}
                      className="w-full bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white focus:border-teal-500"
                    />
                  </div>

                  <div>
                    <label className="block font-semibold text-gray-300 mb-1">Authorised Signatory Designation</label>
                    <input
                      type="text"
                      value={letterhead.authorisedSignatory}
                      onChange={(e) => handleLetterheadChange("authorisedSignatory", e.target.value)}
                      className="w-full bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white focus:border-teal-500"
                    />
                  </div>

                  <div>
                    <label className="block font-semibold text-gray-300 mb-1">Custom Letterhead Footer / Notes</label>
                    <input
                      type="text"
                      value={letterhead.customFooterNotes}
                      onChange={(e) => handleLetterheadChange("customFooterNotes", e.target.value)}
                      className="w-full bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white focus:border-teal-500"
                    />
                  </div>
                </div>
          ...
          {/* Auto-Scheduled Recurring Billing Accordion */}
          <div className="rounded-xl bg-white/5 border border-white/5 overflow-hidden transition-all">
            <button
              type="button"
              onClick={() => setShowScheduledBilling(!showScheduledBilling)}
              className="w-full flex items-center justify-between p-4 bg-white/[0.02] hover:bg-white/5 transition-colors text-left"
            >
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="text-sm font-bold text-white flex items-center gap-2">
                    Auto-Scheduled Recurring Billing
                    {scheduledBilling.isEnabled ? (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 font-bold border border-emerald-500/30">
                        ACTIVE ({scheduledBilling.frequency})
                      </span>
                    ) : (
                  ...
                  <div className="text-xs text-gray-400">
                    Automate recurring invoice generation & customer email dispatches on set frequencies
                  </div>
                </div>
              </div>
              <div className="flex items-center space-x-2 text-gray-400">
                <span className="text-xs text-indigo-400 font-medium">
                  {showScheduledBilling ? "Hide Schedule" : "Configure Schedule"}
                </span>
                {showScheduledBilling ? <BsChevronUp /> : <BsChevronDown />}
              </div>
            </button>

            {showScheduledBilling && (
              <div className="p-5 border-t border-white/10 space-y-4 animate-in slide-in-from-top-2 duration-200">
                {/* Enable Toggle Switch */}
                <div className="flex items-center justify-between p-3 rounded-lg bg-indigo-500/10 border border-indigo-500/20">
                  <div>
                    <div className="text-sm font-semibold text-white">Enable Auto-Scheduled Billing</div>
                    <div className="text-xs text-gray-300">Automatically re-issue this bill on future schedule dates</div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      checked={scheduledBilling.isEnabled}
                      onChange={(e) => setScheduledBilling({ ...scheduledBilling, isEnabled: e.target.checked })}
                ...
                {scheduledBilling.isEnabled && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                    <div>
                      <label className="block font-semibold text-gray-300 mb-1">Recurrence Frequency</label>
                      <select
                        value={scheduledBilling.frequency}
                        onChange={(e) => setScheduledBilling({ ...scheduledBilling, frequency: e.target.value })}
                        className="w-full bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white focus:border-teal-500"
                      >
                    ...
                    <div>
                      <label className="block font-semibold text-gray-300 mb-1">Next Scheduled Billing Date</label>
                      <input
                        type="date"
                        value={scheduledBilling.nextBillingDate}
                        onChange={(e) => setScheduledBilling({ ...scheduledBilling, nextBillingDate: e.target.value })}
                        className="w-full bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white focus:border-teal-500"
                      />
                    </div>

                    <div>
                      <label className="block font-semibold text-gray-300 mb-1">Total Billing Cycles</label>
                      <input
                        type="number"
                        min={1}
                        max={120}
                        value={scheduledBilling.maxBillingCycles}
                        onChange={(e) => setScheduledBilling({ ...scheduledBilling, maxBillingCycles: parseInt(e.target.value) || 12 })}
                        className="w-full bg-[#1e1e1e] border border-white/10 rounded-lg px-3 py-2 text-white focus:border-teal-500"
                      />
                    </div>

                    <div className="md:col-span-3 flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/5">
                      <div>
                        <div className="text-xs font-semibold text-gray-200">Auto Email Dispatch to Customer</div>
                        <div className="text-[11px] text-gray-400">Send PDF copy automatically to customer email when triggered</div>
                      </div>
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={scheduledBilling.autoSendEmail}
                          onChange={(e) => setScheduledBilling({ ...scheduledBilling, autoSendEmail: e.target.checked })}
                          className="sr-only peer"
                        />
