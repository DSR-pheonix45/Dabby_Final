...
export default function Integrations() {
  ...
  // Organization Selector Modal State
  ...
  const fetchConnectionStatus = async () => {
    setLoading(true);
    try {
      const data = await apiJson(`/api/integrations/zoho/status?workbench_id=${activeWorkbench.id}`);
      if (data.connected) {
  ...
  const handleManualSync = async () => {
    ...
    try {
      const res = await apiFetch("/api/integrations/zoho/sync", {
        method: "POST",
      ...
      if (res.ok && result.status === "success") {
        toast.success(`Sync complete! ${result.records_imported} records imported to UFG.`, { id: toastId });
        fetchConnectionStatus();
      } else {
        toast.error(`Sync failed: ${result.detail || "Unknown error"}`, { id: toastId });
      }
    } catch (err) {
      toast.error("Failed to connect to sync engine.", { id: toastId });
    } finally {
      setSyncing(false);
    }
  };
  ...
  const handleDisconnect = async () => {
    ...
    try {
      const res = await apiFetch(`/api/integrations/zoho/disconnect?workbench_id=${activeWorkbench.id}`, {
        method: "POST"
      });
      if (res.ok) {
  ...
  const startZohoOAuthOrOpenModal = async () => {
    setFetchingOrgs(true);
    try {
      // 1. Get OAuth Auth URL from Backend
      const authRes = await apiJson(`/api/integrations/zoho/auth-url?redirect_uri=${encodeURIComponent(window.location.origin + "/oauth/callback")}`);

      if (authRes.auth_url && !authRes.auth_url.includes("MOCK_CLIENT_ID")) {
        // Redirect to real Zoho OAuth consent screen
        window.location.href = authRes.auth_url;
        return;
      }

      // 2. Fallback / Dev Mode: Fetch organizations directly for selection
      const orgsRes = await apiJson(`/api/integrations/zoho/organizations?access_token=mock_access_token`);
      setAvailableOrgs(orgsRes.organizations || []);
      setIsOrgModalOpen(true);
    } catch (err) {
      console.warn("Could not initiate Zoho OAuth URL, opening selector modal:", err);
      setAvailableOrgs([
        {
          organization_id: "90001827",
          name: `${activeWorkbench.name} (Primary Zoho Org)`,
          currency_code: activeWorkbench.currency || "INR",
          country: activeWorkbench.country || "India"
        },
        {
          organization_id: "90001899",
          name: "Global Operations Pvt Ltd (Zoho)",
          currency_code: "USD",
  ...
  const handleConfirmOrgConnect = async () => {
    if (!selectedOrgId && !customOrgName.trim()) {
      toast.error("Please select or enter a Zoho Organization.");
      return;
    }

    const targetOrg = availableOrgs.find((o) => o.organization_id === selectedOrgId);
    const orgId = targetOrg ? targetOrg.organization_id : "9000" + Math.floor(1000 + Math.random() * 9000);
    const orgName = targetOrg ? targetOrg.name : customOrgName.trim() || `${activeWorkbench.name} (Zoho Org)`;

    setSyncing(true);
    const toastId = toast.loading(`Binding ${orgName} 1:1 to Workbench...`);
    try {
      const res = await apiFetch("/api/integrations/zoho/connect", {
        method: "POST",
        body: JSON.stringify({
          workbench_id: activeWorkbench.id,
          provider_org_id: orgId,
          provider_org_name: orgName,
          access_token: "access_token_active",
          refresh_token: "refresh_token_active",
      ...
      if (res.ok) {
        toast.success(`Successfully mapped ${orgName} 1:1 to ${activeWorkbench.name}!`, { id: toastId });
        setIsOrgModalOpen(false);
        fetchConnectionStatus();
      } else {
        toast.error(data.detail || "1:1 Connection binding failed.", { id: toastId });
      }
    } catch (err) {
      toast.error("Failed to map organization.", { id: toastId });
    } finally {
      setSyncing(false);
    }
  };
    ...
    <div className="p-6 lg:p-10 font-dm-sans text-white max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="flex items-center space-x-3 w-full md:w-auto justify-end">
            {connectionData ? (
              ...
              <button
                onClick={startZohoOAuthOrOpenModal}
              ...
              >
                ...
                {fetchingOrgs ? "Opening Zoho OAuth..." : "Connect Zoho Books"}
              </button>
            )}
          </div>
        </div>
      ...
      {isOrgModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#111111] border border-white/10 rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 bg-red-500/20 border border-red-500/30 text-red-400 rounded-xl flex items-center justify-center font-bold text-lg">
                  Z
                </div>
                <div>
                  <h2 className="text-lg font-bold text-white">Select Zoho Organization</h2>
                  <p className="text-xs text-gray-400">Map 1:1 to <span className="text-teal-400 font-medium">{activeWorkbench?.name}</span></p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 space-y-4">
              <div className="space-y-3">
                <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider block">Discovered Organizations</span>
                  ...
                  <div
                    ...
                    className={`p-4 rounded-xl border cursor-pointer transition-all flex items-center justify-between ${
                      selectedOrgId === org.organization_id
                        ...
                        : "bg-[#181818] border-white/10 text-gray-300 hover:border-white/20"
                    }`}
                  >
                    <div>
                      <div className="font-semibold text-sm text-white flex items-center gap-2">
                        <BsBuildingCheck className="text-teal-400" />
                        {org.name}
                      </div>
                      <div className="text-xs text-gray-500 mt-1 font-mono">Org ID: {org.organization_id} • {org.currency_code} ({org.country})</div>
                    </div>
                    {selectedOrgId === org.organization_id && (
                      <BsCheckCircleFill className="text-teal-400 w-5 h-5" />
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="pt-2 border-t border-white/10">
                <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">Or enter Custom Organization Name</span>
                <input
                  type="text"
                  placeholder="e.g. Acme Enterprises (Zoho Books)"
                  value={customOrgName}
                  onChange={(e) => {
                    setCustomOrgName(e.target.value);
                    setSelectedOrgId("");
                  }}
                  className="w-full bg-[#181818] border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-teal-500 transition-colors"
                />
              </div>
            </div>

            <div className="p-6 border-t border-white/10 bg-[#141414] flex justify-end space-x-3">
              <button
                onClick={() => setIsOrgModalOpen(false)}
              ...
              <button
                onClick={handleConfirmOrgConnect}
                disabled={syncing || (!selectedOrgId && !customOrgName.trim())}
                className="px-6 py-2 bg-teal-500 hover:bg-teal-400 disabled:opacity-40 text-black font-bold text-sm rounded-xl transition-all flex items-center gap-2"
              >
                {syncing ? "Mapping & Syncing..." : "Connect & Map 1:1"}
              </button>
            </div>
          </div>
        </div>
