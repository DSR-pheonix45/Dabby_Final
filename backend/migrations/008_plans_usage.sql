...
CREATE TABLE IF NOT EXISTS workbench_usage (
    ...
    UNIQUE (workbench_id, period, metric)
);
...
CREATE TABLE IF NOT EXISTS ai_usage (
    ...
    UNIQUE (workbench_id, user_id, usage_date)
);

CREATE INDEX IF NOT EXISTS idx_workbench_usage_lookup ON workbench_usage(workbench_id, period, metric);
CREATE INDEX IF NOT EXISTS idx_ai_usage_lookup        ON ai_usage(workbench_id, user_id, usage_date);
...
ALTER TABLE workbench_usage ENABLE ROW LEVEL SECURITY;
ALTER TABLE ai_usage ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'workbench_usage' AND policyname = 'members_read_usage') THEN
        CREATE POLICY members_read_usage ON workbench_usage FOR SELECT
