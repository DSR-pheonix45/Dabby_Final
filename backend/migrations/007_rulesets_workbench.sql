...
ON rulesets(user_id, document_type) 
WHERE (status = 'Active');
...
CREATE POLICY "Users can manage rulesets in their workbenches" ON rulesets
    FOR ALL USING (EXISTS (SELECT 1 FROM workbenches WHERE id = rulesets.user_id));
...
CREATE POLICY "Users can manage ruleset versions" ON ruleset_versions
    FOR ALL USING (EXISTS (
        ...
        AND EXISTS (SELECT 1 FROM workbenches WHERE id = rulesets.user_id)
    ));
...
CREATE POLICY "Users can manage financial events" ON financial_events
    FOR ALL USING (EXISTS (
        ...
        AND EXISTS (SELECT 1 FROM workbenches WHERE id = user_documents.user_id)
    )));
