...
def backfill_labels():
    # 1. Get all users
    wb_res = supabase.table("users").select("id").execute()
    users = wb_res.data
    ...
    for wb in users:
        ...
        for pillar_type in ["asset", "liability", "equity", "income", "revenue", "expense"]:
            ...
            labels_res = supabase.table("labels").select("id").eq("user_id", wb_id).in_("type", type_to_check).eq("is_shadow", False).execute()
            
            if not labels_res.data:
                print(f"Seeding essential {pillar_type} labels for workbench {wb_id}...")
                # Fetch sub-accounts for this type
                sub_res = supabase.table("coa_accounts").select("name, type").eq("user_id", wb_id).in_("type", type_to_check).eq("level", 2).execute()
                if sub_res.data:
