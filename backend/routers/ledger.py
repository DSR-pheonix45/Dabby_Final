...
class LabelCreate(BaseModel):
    ...
    custom_sub_type: Optional[str] = None
    description: Optional[str] = None
    current_amount: Optional[float] = 0.0

class LabelUpdate(BaseModel):
    full_account_name: Optional[str] = None
    custom_sub_type: Optional[str] = None
    description: Optional[str] = None
...
async def get_master_accounts():
    try:
        res = supabase.table("master_accounts").select("*").eq("is_active", True).execute()
        return res.data
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/master-sub-accounts")
async def get_master_sub_accounts():
    try:
        res = supabase.table("master_sub_accounts").select("*").eq("is_active", True).execute()
        return res.data
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
