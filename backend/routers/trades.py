...
async def execute_trade_activities(trade_id: str, payload: Dict = None):
    try:
        ...
        if not activities:
            # Dynamically generate activities for legacy trades
            from services.trade_service import trade_service
            trade_service.generate_activities_for_trade(trade_id)
            
            # Re-fetch from DB so activities have proper UUIDs
            act_res = supabase.table("trade_activities").select("*").eq("trade_id", trade_id).order("sequence").execute()
            activities = act_res.data or []
            
        if not activities:
            raise HTTPException(status_code=400, detail="No activities generated for this trade. Please ensure the database migration (006_financial_engine.sql) has been applied.")
        ...
        for act in activities:
            ...
            if not act.get("id"):
                # Skip activities without IDs (insertion failed)
        ...
        if not executed_list:
            raise HTTPException(status_code=400, detail="No activities could be executed. Check that the database migration has been applied.")
        
        # Post strict double-entry using Accounting Compiler
        ...
        supabase.table("trades").update({"status": "Settled", "updated_at": datetime.utcnow().isoformat()}).eq("id", trade_id).execute()
